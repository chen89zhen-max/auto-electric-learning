...
export function E06Experience({ onReturnLobby }: E06ExperienceProps) {
  ...
  const guidance = E06_STAGE_CONTENT[currentStep];



  const handleStepComplete = (step: E06Step, evidence: Record<string, unknown>) => {
  ...
  const handleAdvanceStep = () => {
    if (currentStep === 'MAGNETO_VS_HALL_COGNITION') {
      setCurrentStep('MULTIMETER_AND_OSCILLOSCOPE_TEST');
      setHintRequested(false);
    } else if (currentStep === 'MULTIMETER_AND_OSCILLOSCOPE_TEST') {
      setCurrentStep('SPEED_FREQUENCY_AND_GAP_CALC');
      setHintRequested(false);
    } else if (currentStep === 'SPEED_FREQUENCY_AND_GAP_CALC') {
      setCurrentStep('BLIND_SPEED_SENSOR_FAULT_DIAGNOSIS');
      setHintRequested(false);
    } else if (currentStep === 'BLIND_SPEED_SENSOR_FAULT_DIAGNOSIS') {
  ...
  return (
    <main className="app-shell level03-shell e06-shell">
      {/* Top Navigation Bar */}
      <header className="topbar">
        <div className="brand-lockup">
          ...
          <div>
            <p className="eyebrow">篇章五：让电路感知、判断和执行 · 传感器基石</p>
            <h1>E06 转速信号寻踪——转速传感器与信号调理</h1>
          </div>
        </div>
      ...
      <nav className="training-stage-stepper px-6 py-2.5 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between text-sm overflow-x-auto">
        {(
          [
            ...
            { id: 'MULTIMETER_AND_OSCILLOSCOPE_TEST', num: '2', name: '万用表示波器实测' },
            { id: 'SPEED_FREQUENCY_AND_GAP_CALC', num: '3', name: '转速频率与气隙' },
            { id: 'BLIND_SPEED_SENSOR_FAULT_DIAGNOSIS', num: '4', name: '传感器盲测排查' },
